...
deResults$down_label <- ifelse(deResults$gene %in% c("Nfkbia", "Nfkbiz", "Tnfaip3", "Myc", "Nfkbid", "Fosb", "Klf10", "Fosl1", "Fos"), TRUE, FALSE)

# results$genelabels <- ifelse(results$Gene %in% c("IL23A", "IL1A","IL6", "CD80","CD86","NFKB","BAFT2"), 
#                              as.character(results$Gene), NA)

ggplot(deResults, aes(x = log2FoldChange, y = negLog10FDR)) + 
  geom_point(color = "darkgray") +
  geom_vline(xintercept=-1, linetype = "dashed") + xlim(-2.5,1.1) + ylim(-1,25)+
  geom_hline(yintercept=1, col = "black", linetype = "dashed") + 
  geom_point(data = deResults %>% filter(negLog10FDR > 1 & log2FoldChange <(-1)), color = "darkred") +
  theme_minimal() + 
  ylab("-Log10FDR")
```
